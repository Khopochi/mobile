...
export const Forgot = () => {
    ...
    const handleInputChange = (e) => {
      ...
      const sendCode = async () => {
        ...
        if(value.length <= 0){
            ...
            const tempuser = {
              ...
              const res = await axios.post(process.env.REACT_APP_API_URL+"user/reset", tempuser)
              setLoader(false)
      ...
      const onSubmit = async () => {
        ...
        dispatch({type:"LOGIN_START"})
        try {
          const res = await axios.post(process.env.REACT_APP_API_URL + "user/update/"+value1+"/"+value);
      ...
      const onRest = async () => {
        ...
        dispatch({type:"LOGIN_START"})
        try {
            const res = await axios.put(process.env.REACT_APP_API_URL + "user/updateuser/"+value, credentials);
            if(res.data._id){
                              dispatch({type:"LOGIN_SUCCESS", payload: res.data})
                              // console.log(res.data)
                              setLoader(false)
